...
class Session {
	...
	public function __construct($config) {		
		...
		if (isset($this->_config['handler']) && $this->_config['handler']) {
			$handlerClass = 'Ming_Session_Handler_' .ucfirst($this->_config['handler']);
			...
			if (\Flywheel\Config\Handler::get('debug')) {
				\Flywheel\Debug::mark('Installed '.$handlerClass, get_class($this));				
			}
			//<-- /end debug code -->		
		}
        ...
        if (isset($this->_config['cookie_ttl']) 
            ...
            $domain = (isset($this->_config['cookie_domain'])) ? 
            				$this->_config['cookie_domain'] : '.'. Ming_Factory::getRouter()->getDomain();
	...
	public function start() {
		...
		$this->_setTimers();
		$this->_validate();
        \Flywheel\Debug::mark('started session', 'system.' .get_class($this), 'system');
	...
	protected function _validate() {
		...
		if (null === $ip) {
			...
			throw new \FlyWheel\Session\Session_Exception('Session: Invalid session address');
		}
	}
