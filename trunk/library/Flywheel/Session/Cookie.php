...
class Cookie {
	 ...
	 * 
	 * @param string	$name
	 * @param integer	$type \Flywheel\Filter\Input::TYPE_* @see \Flywheel\Filter\Input
	 * @param mixed		$default
	 * 
	 * @return mixed
	 ...
	 * @throws Exception
	 */
	public function readSecure($name, $type = Input::TYPE_STRING, $default = null) {
		...
		if ($value[1] > time()) {
			if (true == $this->_exception) {
				throw new \Flywheel\Exception('Invalid cookie timestamp!');
			}
			return null;
		}
