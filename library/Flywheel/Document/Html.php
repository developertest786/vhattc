...
class Html extends BaseDoc {
	...
	public function css() {
        ...
        $cssv = \Flywheel\Config\Handler::get('css_version');
		if (\Flywheel\Config\Handler::get('compile_css')) {
	...
	public function js($pos = 'BOTTOM') {
        $jsv = \Flywheel\Config\Handler::get('js_version');
		$pos = strtoupper($pos);
		$jsCode = '';
		...
		if (isset($this->_javascript[$pos])) {
			if (\Flywheel\Config\Handler::get('compile_js')) {
