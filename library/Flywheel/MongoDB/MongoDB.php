...
class MongoDB
{
    ...
    public function __construct($config = array(), $connect = true)
    {
        ...
        {
            $mongo_config = \Flywheel\Config\Handler::load('global.config.mongodb', 'mongodb');
            $params = $mongo_config;
    ...
    {
        if (empty($dsn)) {
            throw new Exception('To switch MongoDB databases, a DSN must be specified');
        }
     ...
     *
     * @access public
     * @return \Flywheel\MongoDB\MongoDB
     */
    ...
    {
        if (empty($collection)) {
            throw new Exception('No MongoDB collection specified to add index to');
        }
