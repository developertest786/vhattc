...
(function ($) {
    ...
    function Carousel(root, options) {
        ...
        function initialize() {
            ...
            /* hoangnm: fill height cho viewport. Không fix height cho viewport
            * Nếu slide ngang(cuộn dọc chưa viết)
            * */
            if(bAxis) {
                oContent.css("position", "relative");/* oContent: ul.overview */
                oViewport.css("height",oContent.outerHeight());
                oContent.css("position", "absolute");
                /*fill height các item*/
                var paddingTopBottomValue = $(oPages[0]).outerHeight() - $(oPages[0]).height();
                console.log("paddingTopBottomValue: " + paddingTopBottomValue);
                oPages.css("height", oViewport.height() - paddingTopBottomValue);
            }
            oSelf.move(1);
            setEvents();
            return oSelf;
